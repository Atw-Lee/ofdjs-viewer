...
import './style.css';
function App() {
  const [source, setSource] = useState<OFDSource>('/sample.ofd'), [name, setName] = useState('sample.ofd'), [drag, setDrag] = useState(false);
  ...
  return <main onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); open(e.dataTransfer.files[0]); }}>
    <header><a href="/" className="logo">▤ <strong>ofd<span>js</span></strong><small>DOCUMENT TOOLKIT</small></a><span className="version">v0.1 · Canvas engine</span></header>
    ...
    <footer><span>OFD.js — JavaScript 文档引擎与 React 阅读器</span><button onClick={() => { setSource('/sample.ofd'); setName('sample.ofd'); }}>打开示例文档 ↗</button></footer>
  </main>;
}
